...
if ($is_logged_in && $is_claimed) {
    ...
    if (!empty($claimed_at)) {
        try {
            ...
            $end_at = (clone $start_at)->modify('+14 days')->setTime(23, 59);
    ...
    <main class="nm-simple-page">
        <div class="container nm-container">
            <section class="nm-simple-coupon">
                <p class="nm-simple-coupon-code">騎士節活動</p>
                <p class="nm-simple-coupon-title">全館商品 8 折</p>
            ...
            <section class="nm-simple-list">
                ...
                <div class="nm-simple-list-item">
                    <h2>優惠內容</h2>
                    <p>使用優惠碼 <?php echo htmlspecialchars($coupon_code); ?>，可享全館商品 8 折。</p>
                </div>
                <div class="nm-simple-list-item">
                    <h2>使用條件</h2>
                    <p>每個會員帳號限領一次，僅限活動期間內可套用。</p>
                </div>
                <div class="nm-simple-list-item">
                    <h2>付款方式</h2>
                    <p>依結帳頁可用付款方式為準，折扣結果以系統計算為準。</p>
                </div>
                <div class="nm-simple-list-item">
                    <h2>適用範圍</h2>
                    <p>適用於本網站商品訂單，實際可用條件以結帳頁顯示為準。</p>
                </div>
            </section>
            ...
            <section class="nm-simple-status" id="claim">
                <h2>優惠券狀態</h2>
                <?php if ($is_logged_in): ?>
                    <p>帳號：<?php echo htmlspecialchars($user_profile['username'] ?? ($_SESSION['user_name'] ?? '會員')); ?></p>
                    <?php if ($is_claimed): ?>
                ...
                <?php else: ?>
                    <p>請先登入會員後再領取此優惠券。</p>
                    <div class="nm-simple-actions">
                        <a href="login.php?redirect=<?php echo urlencode('coupon_rider_day.php'); ?>" class="nm-simple-btn nm-simple-btn-primary">前往登入</a>
